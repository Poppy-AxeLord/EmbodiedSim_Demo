...
class HandController:
    ...
    def __init__(self, model, data, kp_finger=2.5, kd_finger=0.05,
        ...
        # 增益已归零 => 执行器力恒为 0，ctrl 不再影响物理（详见 free_builtin）
        self._builtin_neutralized = bool(
            np.all(self.m.actuator_gainprm[:, 0] == 0.0)
            and np.all(self.m.actuator_biasprm[:, 1] == 0.0)
            and np.all(self.m.actuator_biasprm[:, 2] == 0.0)
            and np.all(self.m.actuator_biasprm[:, 0] == 0.0))
    ...
    def free_builtin(self, force=False):
        """让内置位置执行器输出 0 力。

        本工程在 __init__ 里已把 actuator_gainprm[:,0] 与 actuator_biasprm[:,1:3]
        全部置零，而 biasprm[:,0] 对 position 执行器本来就是 0，因此执行器力

            f = gainprm[0]·ctrl + biasprm[0] + biasprm[1]·len + biasprm[2]·vel  ≡ 0

        与 ctrl 取值无关。也就是说这里写 ctrl 是**可证明的无效操作**，而它每步
        要走一遍 20 个执行器的传动映射（约 160 µs）——在 RL 训练里每步都调，
        是整条控制回路最热的一段。这里只在增益没被置零（即调用者自己改过模型）
        时才真正写，否则直接跳过。

        想强制写回（例如调试传动映射）用 force=True。
        """
        if not self._builtin_neutralized or force:
